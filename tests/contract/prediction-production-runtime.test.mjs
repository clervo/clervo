...
test('prediction production runtime degrades to the remaining valid venue without fabricating a second result', async () => {
  const runtime = createPredictionProductionRuntime({ store: store(), now: () => nowMs, async fetcher(input) { if (new URL(input).hostname.includes('kalshi')) return new Response('down', { status: 503, headers: { 'content-type': 'application/json' } }); return Response.json([polymarket]); } });
