...
test('release targets preserve current registry truth and bind the exact release candidate', () => {
  ...
  assert.deepEqual(
    ...
    [
      { registry: 'npm', name: '@clervo/sdk', version: '0.5.0' },
      { registry: 'npm', name: '@clervo/mcp', version: '0.5.0' },
      { registry: 'pypi', name: 'clervo-sdk', version: '0.4.0' },
    ],
  );
...
test('package publishing is manual, commit-bound, environment-protected, and tokenless', () => {
  ...
  const sdkPublish = publishWorkflow.indexOf('npm publish release-artifacts/npm/clervo-sdk-0.5.0.tgz');
  const mcpPublish = publishWorkflow.indexOf('npm publish release-artifacts/npm/clervo-mcp-0.5.0.tgz');
