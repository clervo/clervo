...
test('public Prediction route challenges before validation, settles useful output once, and replays', async (context) => {
  ...
  const probe = await fetch(`${origin}/v1/prediction/execute`, { method: 'POST', headers: edge }); assert.equal(probe.status, 402); assert.equal(probe.headers.has('www-authenticate'), true); assert.equal((await probe.json()).accepts[0].amount, '1000');
  ...
  const paid = await fetch(`${origin}/v1/prediction/execute`, { method: 'POST', headers: { ...headers, 'payment-signature': 'opaque' }, body }); assert.equal(paid.status, 200); const paidBody = await paid.json(); assert.equal(paidBody.result.output.kind, 'markets'); assert.equal(paidBody.receipt.customerCharge.amountAtomic, '1000'); assert.equal(paidBody.receipt.supplierCost.amountAtomic, '0'); assert.equal(paidBody.receipt.requestHash, paidBody.requestHash); assert.equal(paidBody.receipt.settlement.status, 'settled');
