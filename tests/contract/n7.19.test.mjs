...
test('Sandbox system capacity uses two dedicated regular nodes and keeps execution isolated', () => {
  assert.equal(policy.nodePool.name, 'sandbox-system');
  assert.equal(policy.nodePool.machineType, 'e2-standard-2');
  assert.equal(policy.nodePool.nodes, 2);
  assert.equal(policy.controller.replicas, 2);
...
test('capacity control spreads replicas, verifies readiness, and retires only the named undersized pool', () => {
  ...
  assert.match(capacity, /retire:\$\{policy\.retiredNodePool\}/u);
  assert.doesNotMatch(capacity, /ai\.clervo\.dev|instances', 'delete'|clusters', 'delete'/u);
