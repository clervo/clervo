...
expected_browser_image="sha256:552de4dc382af6e73acd746cdb76cc17771e7cd7eb10532df25dc7f5b7e27fa0"
expected_commerce_image="sha256:d4ed7e42f56509dcefa69dd2c4539996574cc1a0677fbf5e9ea27648d0309400"

for required in "${browser_archive}" "${commerce_archive}" "${seccomp_profile}"; do
  test -f "${required}" || { printf 'missing qualification input\n' >&2; exit 90; }
done

docker load --input "${browser_archive}" >/dev/null || exit 91
docker load --input "${commerce_archive}" >/dev/null || exit 92
test "$(docker image inspect clervo-stage4-browser:dev --format '{{.Id}}')" = "${expected_browser_image}" || exit 93
test "$(docker image inspect clervo-stage4-commerce:dev --format '{{.Id}}')" = "${expected_commerce_image}" || exit 94
