...
result = client.ai.execute(
    ...
    maximum_output_tokens=16,  # a fresh operation key is generated by default
)
```
...
Search remains available through `client.search.web()`. The released
`client.search.answer()` compatibility method fails deterministically because
synthesis is not implemented. `recovery_action_for(error)` returns one safe next
