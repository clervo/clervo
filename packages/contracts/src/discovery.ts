import { SEARCH_FREE_PATH, SEARCH_PAID_PATH, SEARCH_PRODUCT_ID } from './search-http.js';
import { CONTRACT_VERSION } from './types.js';

export const DISCOVERY_VERSION = '2026-07-30.2' as const;
...
export interface DiscoveryProduct {
  productId: typeof SEARCH_PRODUCT_ID;
...
export function createOpenApiDocument(schemas: Record<string, Record<string, unknown>>): OpenApiDocument {
  return {
    openapi: '3.1.1',
    jsonSchemaDialect: 'https://json-schema.org/draft/2020-12/schema',
    info: { title: 'Clervo Next search contract', version: CONTRACT_VERSION, description: 'Generated repository-local contract for the bounded search.query HTTP slice. Deployment is not verified and public payment is not implemented.' },
    paths: {
...
export function createDiscoveryDocument(): DiscoveryDocument {
  return {
    ...
    name: 'Clervo Next',
    description: 'Machine-readable contract for the implemented repository-local search.query HTTP slice. Public deployment and real payment are not verified.',
    lifecycle: 'implemented_unverified',
    callable: true,
    payment: { protocol: 'x402', implemented: false, settlementVerified: false },
    ...
    products: [{ productId: SEARCH_PRODUCT_ID, status: 'implemented_unverified', deliveryModes: ['sync'], freeEndpoint: `${PUBLIC_ORIGIN}${SEARCH_FREE_PATH}`, paidEndpoint: `${PUBLIC_ORIGIN}${SEARCH_PAID_PATH}`, payment: { challengeImplemented: true, payable: false, mockExecutionAvailableByInjectionOnly: true } }],
...
export function createLlmsText(): string {
  return ['# Clervo Next', '', '> Repository-local search.query HTTP implementation. Public deployment is not verified and real x402 payment is not implemented.', '', 'Important status:', '', '- Lifecycle: implemented_unverified', '- Callable products: search.query', '- Free endpoint: POST /v1/search/free', '- Paid endpoint: POST /v1/search/paid (non-payable mock challenge by default)', '- x402 payment implementation: not implemented', '- Production deployment: not verified', '', '## Machine-readable contracts', '', `- [OpenAPI contract](${PUBLIC_ORIGIN}/openapi.json): Search request, free-sample, and non-payable paid-challenge paths.`, `- [Catalog](${PUBLIC_ORIGIN}/catalog.json): Versioned search.query publication.`, `- [Discovery document](${PUBLIC_ORIGIN}/.well-known/clervo.json): Explicit implementation and payment limitations.`, `- [JSON Schemas](${PUBLIC_ORIGIN}/schemas/${CONTRACT_VERSION}/): Draft 2020-12 contracts.`, ''].join('\n');
}
...
export function assertPreviewArtifacts(openapi: OpenApiDocument, discovery: DiscoveryDocument, llms: string): void {
  ...
  if (!discovery.callable || discovery.products.length !== 1 || discovery.products[0].productId !== SEARCH_PRODUCT_ID) failures.push('discovery_search_product_required');
  if (discovery.payment.implemented || discovery.payment.settlementVerified || discovery.products[0].payment.payable) failures.push('discovery_must_not_claim_payment');
  if (!llms.includes('Callable products: search.query')) failures.push('llms_missing_callable_product');
