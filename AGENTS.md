...
4. Execute only the exact ticket explicitly authorized by the current handoff.
   A proposed next ticket is not authorization.
...
6. Never begin the next ticket or a later stage automatically. When the active
   ticket is complete, verified, recorded, and committed, report and stop.
...
This autonomy never overrides the master plan, stage order, active ticket,
truthful-claims policy, payment restrictions, secret rules, cost ceilings,
provider terms, or stop conditions.
