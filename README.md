...
Tickets **N0.1** through **N4.22** are complete. N4.22 binds every remaining Stage 4 blocker to exact required/missing evidence and a dependency-ordered N4.23–N4.28 campaign queue. Workspace TypeScript 7.0.2 accepts `ES2023`, so the Mac VS Code warning is an older editor language-service mismatch and required no repository change. The campaign cannot truthfully proceed from this container: the private N4.18 Cloud Run service returns HTTP 403 without authentication, no active gcloud/ADC or provider credentials are available, public Nominatim is non-resale, Common Crawl remains legal-review gated, and payable-route/alert-channel authorization is absent. Canonical acceptance passed 205/205. The Stage 4 verifier remains `blocked` with 21 checks; search is not the reference pattern and Stage 5 is not authorized or started.
