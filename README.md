...
Tickets **N0.1** through **N4.21** are complete. N4.21 ran SearXNG `057a77168d3175ce2e42e5b10f46a8df073886d5` in a dedicated Python virtual environment and one-worker process bound only to `127.0.0.1:18888`, with exactly the Wikipedia and OpenStreetMap/Nominatim engines. Each engine returned one bounded result. A direct Common Crawl `CC-MAIN-2026-30` lookup then authorized one exact 954-byte WARC range whose HTTP 206 `Content-Range`, compressed hash, decoded WARC target, and payload digest matched. This is development evidence only: public Nominatim resale remains prohibited, Common Crawl content use requires legal review, neither path is product-connected or staging-verified, and no general-Web or production-ready claim is made. Focused tests passed 9/9; canonical acceptance passed 200/200 on its sole invocation. The separate closeout Stage 4 verifier passed with decision `blocked`: 21 checks remain blocking, search is not the reference pattern, and Stage 5 is not authorized or started.
