...
must be rechecked when needed because maintenance, license and install
behavior can change.
...
| GitHub CLI | `github.com/cli/cli`; MIT | user-local binary, OAuth/device login, remote GitHub writes | deferred until a remote workflow needs it; native Git retained |
...
| kubectl | `kubernetes.io`; Apache-2.0 | standalone pinned binary; cluster credentials and mutation | deferred until cluster work needs it and external effects are safe |
| Helm | `helm.sh`; Apache-2.0 | standalone pinned binary; chart rendering and cluster writes | deferred until deployment work needs it |
| OpenTofu | `opentofu.org`; MPL-2.0 | standalone binary; state/provider/cloud mutation | preferred over Terraform when needed; deferred |
...
| PostgreSQL client | PostgreSQL project; PostgreSQL License | apt/container client; database credentials and writes | deferred until a database diagnostic needs it |
...
| Obsidian | Obsidian proprietary application | GUI/plugin vault access, separate documentation store | deferred; Git documentation is canonical |

## Design, browser and visual QA

| Candidate | Official source / license | Result |
| --- | --- | --- |
| Impeccable | `github.com/pbakaus/impeccable`; Apache-2.0 | reviewed, deferred; compact Clervo design skill avoids another catalog until measured work proves value |
| Figma MCP | official Figma remote endpoint/terms | prepared disabled; requires owner OAuth and exact file authority |
| shadcn | `ui.shadcn.com`; MIT CLI, registry/network and repository writes | version 4.16.1 audited, deferred until frontend work needs it |
...
These remain project-local choices to make when needed, not global installs:

| Capability | License/terms reviewed | Decision |
| --- | --- | --- |
| Next.js, React | MIT | defer until frontend implementation needs them |
