...
| Field | Value |
|---|---|
| Current milestone | **B10 — IN PROGRESS. Search and Sandbox are deployed; the repaired guarded proof is live and externally verified before payment. Stop for the owner's first Search MetaMask interaction. B11 is not started.** |
| Milestone status | `b10_guarded_proof_live_preflight_verified_owner_search_signature_waiting`. **PROVEN 2026-08-10:** the prior browser 405 is fully reconciled as challenge-only and unfunded. Worker version `53d3cace-9578-4823-85de-1ac773ff5ade` now serves 100% of both proof routes from exact candidate `a5e8502`. Both public pages and both hostnames load; guarded configuration pins payer, receiver, native Base USDC, chain 8453, CDP facilitator, exact 6000/10000-atomic prices, and fresh `20260810c` keys. The served bundle is byte-identical to the candidate. An externally loaded browser with all paid-operation traffic locally intercepted proved each page produces one signed POST, one unsigned challenge request, and one unsigned replay—no automatic payment retry. Production received no Search/Sandbox request after deployment; Base still shows zero payer outflow and receiver inflow after B9. No payment, execution, settlement, receipt, or accounting effect has occurred. |
...
| Latest commit | B10 guarded-proof deployment checkpoint; use `git log -1` for its exact SHA. The deployed immutable Worker source is `a5e850298bf8a489892a92f6935ac4381fc94612`. |
| Current production release | Cloud Run origin `clervo-api-production-00043-zag` at 100% traffic, image `sha256:73fcc7d64a2b3bc1c7010210bbb78133f276148fc8195f75d53ae90260031e4c`, `CLERVO_RELEASE_ID=3dfe8a629d724f72f41174aad5ace8f5e7eb8927`; API edge worker `clervo-api-edge-production` version **`7a562b61-1d66-4468-9896-c457e99914d7`**; site worker `clervo-site-production` version **`a9f8b31c-eda2-401d-a274-4c2b2969aba8`**; temporary guarded proof worker `clervo-b10-proof-temporary` version **`53d3cace-9578-4823-85de-1ac773ff5ade`** at 100%. API, secrets, DNS, and the other workers were unchanged by the proof deployment. |
...
| Current blockers | The owner must perform the first interactive MetaMask action on the Search proof page; no wallet material enters this environment. The pinned payer `0x1ada…9207` currently holds 9000 atomic Base USDC: enough for Search alone, but below the 16000-atomic two-proof total and below Sandbox after Search. No funding action is authorized or attempted by this repository. |
...
| Owner approvals waiting | Deployment approval was consumed exactly for proof Worker version `53d3cace-9578-4823-85de-1ac773ff5ade` and caused zero payment effects. The next action is the owner's interactive Search MetaMask flow. The B10 funded proof remains bounded to Search 6000 plus Sandbox SHORT 10000 atomic USDC, maximum 16000 total and two funded execution effects, with no automatic payment retry. |
...
| Exact next task | **STOP FOR OWNER INTERACTION.** Open `https://clervo.dev/proof/b10-search/` first. Connect only the pinned payer, verify the displayed challenge, and approve at most the one 0.006-USDC Search authorization. Do not open or sign Sandbox yet. |
| Files and services for that task | Only `clervo-b10-proof-temporary` and its two `/proof/b10-*` pages are in deployment scope. API revision `00043-zag`, API edge/site workers, secrets, DNS, payment configuration, protected AI infrastructure, and the frozen B12 visual system remain unchanged. |
...
2. **Status:** `in_progress` — production Search/Sandbox hardening is deployed on
   release `3dfe8a6`. The 2026-08-09 browser 405 is directly reconciled as a
   pre-signature, challenge-only failure with zero funded or execution effect.
   The guarded repair is live and externally verified before payment; neither
   funded proof has been attempted after reconciliation. The next action is the
   owner's interactive Search MetaMask flow.
...
6. **Current evidence:** Search free path is `externally_repeated`; both paid
   ...
   failed browser proof created only expired durable `challenged` rows: no
   payment fingerprint, execution, settlement, accounting, receipt, charge, or
   Base transfer. The six-entry receiver ledger remains unchanged from B9.
