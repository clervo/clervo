...
| Field | Value |
|---|---|
| Current milestone | **B10 — IN PROGRESS. Search and Sandbox are deployed; the guarded funded proof is reconciled after a pre-signature HTTP 405 and its immutable repair candidate awaits owner deployment approval. B11 is not started.** |
| Milestone status | `b10_405_reconciled_candidate_pending_owner_deploy_approval`. **PROVEN 2026-08-10:** the browser attempt reached the B10 Search challenge once, then the payment wrapper discarded its `Request` and issued a `GET`; the API edge returned 405 before x402 payload creation. Direct managed PostgreSQL reconciliation found both original and rotated Search/Sandbox keys only in `challenged`, with no payment fingerprint, execution, settlement, response, receipt, or receiver-accounting row. The receiver ledger remains at the balanced six-entry B9 head. Base shows no payer outflow or receiver inflow after B9. Therefore the 405 caused zero wallet signatures, x402 authorizations, Search/Sandbox executions, settlements, transfers, ledger entries, durable paid outcomes, receipts, or charges. The replacement deliberately uses fresh reconciled keys, restores the guarded proxy, pins the known approved payer/facilitator, and emits exactly one signed request with no automatic payment retry. It is not deployed. |
...
| Latest commit | B10 guarded-proof repair candidate; use `git log -1` for the exact immutable SHA after the candidate commit. |
| Current production release | Cloud Run origin `clervo-api-production-00043-zag` at 100% traffic, image `sha256:73fcc7d64a2b3bc1c7010210bbb78133f276148fc8195f75d53ae90260031e4c`, `CLERVO_RELEASE_ID=3dfe8a629d724f72f41174aad5ace8f5e7eb8927`; API edge worker `clervo-api-edge-production` version **`7a562b61-1d66-4468-9896-c457e99914d7`**; site worker `clervo-site-production` version **`a9f8b31c-eda2-401d-a274-4c2b2969aba8`**; temporary guarded proof worker `clervo-b10-proof-temporary` version **`3dbbdbf8-36f7-4b51-98bd-119f1d8c293c`**. No production mutation has followed the reconciled 405. |
...
| Owner approvals waiting | Deploy the single immutable guarded proof Worker candidate after reviewing its SHA/version/diff/rollback. The B10 funded proof remains bounded to Search 6000 plus Sandbox SHORT 10000 atomic USDC, maximum 16000 total and two funded execution effects, with no automatic payment retry. Each MetaMask signature remains interactive and owner-controlled. |
...
| Exact next task | Upload the immutable guarded proof Worker candidate without deploying it, report its exact version and rollback, and stop for owner deployment approval. After approval, externally prove both guarded pages and stop for the owner's first Search MetaMask signature. |
...
2. **Status:** `in_progress` — production Search/Sandbox hardening is deployed on
   ...
   A guarded proof Worker repair candidate awaits owner deployment approval;
   neither funded proof has been attempted after reconciliation.
