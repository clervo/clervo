...
const assets = sources.flatMap((source) => source.modelIds.map((modelId) => {
  ...
  return {
    ...
    modelId,
    product: productId,
    listingStatus: source.serviceId === 'supply.mistral' ? 'priced_evaluation_only' : source.serviceId === 'supply.cohere' ? 'priced_unavailable_trial_limit' : source.serviceId === 'supply.cerebras' || isZai || (isSambanova && modelId === 'MiniMax-M2.7') ? 'priced_unavailable_no_balance' : ['supply.nvidia', 'supply.openrouter', 'supply.sambanova', 'supply.siliconflow'].includes(source.serviceId) ? 'priced_terms_blocked' : 'priced_pending_qualification',
    ...
    termsStatus: ['supply.nvidia', 'supply.openrouter', 'supply.sambanova', 'supply.siliconflow'].includes(source.serviceId) ? 'blocked' : isZai || ['supply.cohere', 'supply.mistral'].includes(source.serviceId) ? 'restricted' : 'unreviewed',
  };
