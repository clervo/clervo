...
const capabilities = [
  ...
  {
    capabilityId: 'search.web', publicAssets: priced('search-supply-pricing.v1.json', ({ publicSellable }) => publicSellable === true), lifecycle: 'production', routeState: 'qualified_multi_source_primary_preference_with_controlled_degradation', primaryServices: ['supply.research_federation'], fallbackServices: ['supply.gdelt_news', 'supply.google_news_rss', 'supply.github_public', 'supply.npm_registry', 'supply.openalex', 'supply.federal_register', 'supply.socrata', 'supply.wikimedia_open_data', 'supply.crossref_open_metadata'], pricingCatalogs: [ref('search-supply-pricing.v1.json')], quality: 'good', termsStatus: 'approved', quotaRunway: 'Credential-free public APIs with hard per-operation source/page/byte/deadline ceilings, source-level failure isolation, canonical deduplication, primary-source ranking, and no automatic paid overage. Exa, Parallel, Tavily, Firecrawl and Brave/Serper remain technically stronger optional suppliers pending owner-controlled credentials and commercial review.', healthMethod: ['tests/contract/search-supply-routing.test.mjs', 'tests/contract/b10-search-sandbox.test.mjs', 'services/search/src/research-pipeline.ts'], secretLocations: ['none:credential_free_research_federation'], replacementPlan: 'Add an owner-approved semantic search or deep-research supplier only when credentials and resale terms are explicitly approved; preserve this federation as an independent no-key fallback.'
  },
  {
