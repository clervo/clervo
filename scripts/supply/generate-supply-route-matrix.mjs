...
const capabilities = [
  ...
  {
    capabilityId: 'rpc.multichain', publicAssets: priced('rpc-supply-pricing.v1.json'), lifecycle: 'internal_only', routeState: 'blocked_production_terms', primaryServices: [], fallbackServices: [], pricingCatalogs: [ref('rpc-supply-pricing.v1.json')], quality: 'mixed', termsStatus: 'blocked', quotaRunway: 'Twenty no-key reads are technically healthy but lack production rights, dedicated quotas, or SLA; the owned Solana allocation prohibits third-party benefit.', healthMethod: [evidence('public-rpc-mesh-qualification.v1.json'), evidence('public-rpc-terms-review.v1.json'), evidence('helius-qualification.v1.json')], secretLocations: ['none:NO_CUSTOMER_ROUTE'], replacementPlan: 'Qualify the selected dedicated free-tier multi-chain account with per-key hard limits, then add an independent family for critical networks.'
  },
  {
...
const ownerBlockers = [
  { blockerId: 'owner.drpc_account', requiredAction: 'Complete the selected multi-chain RPC account login and provide a dedicated free-tier key with paid deposits and overage disabled.', unblocks: ['rpc.multichain'], spendingAuthorized: false },
