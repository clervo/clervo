...
const capabilities = [
  ...
  {
    capabilityId: 'blockchain.data', publicAssets: priced('blockchain-data-supply-pricing.v1.json'), lifecycle: 'mixed', routeState: 'selected_adapter_ready_owner_credential', primaryServices: [], fallbackServices: [], pricingCatalogs: [ref('blockchain-data-supply-pricing.v1.json')], quality: 'mixed', termsStatus: 'mixed', quotaRunway: 'The selected no-card multichain source advertises 100,000 daily credits and 5 RPS with a 100,000-call hard ceiling. The existing local-development source remains terms-blocked and is not a customer fallback.', healthMethod: [evidence('blockscout-market-preflight.v1.json'), evidence('zerion-qualification.v1.json'), 'tests/contract/blockchain-data-adapter.test.mjs'], secretLocations: ['none:BLOCKSCOUT_API_KEY_MISSING', 'legacy_import:ZERION_API_KEY'], replacementPlan: 'Create one dedicated no-card key, qualify normalized EVM wallet, token, and transaction routes, and retain raw RPC and protocol-directory gaps as separate scopes.'
  },
  {
    capabilityId: 'storage.object', publicAssets: priced('storage-supply-pricing.v1.json'), lifecycle: 'production', routeState: 'adapter_ready_blocked_owner_credential', primaryServices: [], fallbackServices: [], pricingCatalogs: [ref('storage-supply-pricing.v1.json')], quality: 'unranked', termsStatus: 'restricted', quotaRunway: 'Published recurring free allowance and local operation ceilings are known, but account usage and automatic overage state are not exposed by S3.', healthMethod: [evidence('cloudflare-r2-qualification.v1.json'), evidence('r2-control-plane-diagnostic.v1.json'), 'tests/contract/object-storage-adapter.test.mjs'], secretLocations: ['legacy_import:R2_ACCESS_KEY_ID,R2_SECRET_ACCESS_KEY'], replacementPlan: 'Install one least-privilege key for a dedicated bucket, then run the prepared synthetic lifecycle qualifier; the tenant-isolated signed adapter and hard per-process ceilings are already complete.'
  },
  {
...
const ownerBlockers = [
  ...
  { blockerId: 'owner.blockscout_account', requiredAction: 'Create one no-card multichain data account, generate a dedicated API key, and install it outside chat and Git as BLOCKSCOUT_API_KEY.', unblocks: ['blockchain.data'], spendingAuthorized: false },
