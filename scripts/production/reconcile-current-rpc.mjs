...
// Read-only reconciliation for the single B14 production RPC payment. This
// script never signs, settles, retries, or mutates production.
...
const idempotencyKey = process.env.CLERVO_RPC_PROOF_IDEMPOTENCY_KEY;
const expectedChargeAtomic = '1000';
...
try {
  ...
  assert.equal(response.productId, 'rpc.call');
  assert.equal(response.state, 'RECEIPTED');
  assert.equal(response.requestHash, row.request_hash);
  assert.equal(response.result?.output?.kind, 'rpc');
  assert.equal(response.result.output.chainId, 'eip155:1');
  assert.deepEqual(response.result.output.outcomes, [{ id: 1, ok: true, result: '0x1' }]);
  ...
  const referenceHash = row.settlement_json?.referenceHash;
  assert.match(referenceHash ?? '', /^sha256:[a-f0-9]{64}$/u, 'settlement reference missing');
  ...
  const transferLogs = await client.getLogs({
    ...
    args: { to: receiver },
    fromBlock: head > 512n ? head - 512n : 0n,
    toBlock: head,
    strict: true,
  });
  const matchedReferences = transferLogs.filter(({ args, transactionHash }) => (
    args.value === BigInt(expectedChargeAtomic)
    && hashJson({ network: 'eip155:8453', transaction: transactionHash.toLowerCase() }) === referenceHash
  ));
  assert.equal(matchedReferences.length, 1, 'settlement reference did not identify one exact receiver transfer');
  const transaction = matchedReferences[0].transactionHash.toLowerCase();
  const chainReceipt = await client.getTransactionReceipt({ hash: transaction });
  ...
  process.stdout.write(`${JSON.stringify({
    schemaVersion: 'clervo.rpc-current-reconciliation.v1',
    ...
    operation: {
      ...
      chainId: response.result.output.chainId,
      outcomeCount: response.result.output.outcomes.length,
      accountingEntryId: entry.entry_id,
      accountingEntryHash: entry.entry_hash,
    },
    onchain: { network: 'eip155:8453', receiptStatus: 'success', exactReceiverTransferCount: exactTransfers.length, blockNumber: chainReceipt.blockNumber.toString() },
    ...
    transactionMaterialLogged: false,
