...
// The public discovery surface deliberately omits internal grading (proof level,
// families we do not offer). The site is a first-party surface and may still read
// the internal registry for pages that show our own maturity view, so those pages
// keep working without that grading re-entering a public artifact.
import siteRegistrySource from '../../../generated/site/site-registry.json';
import launchStateSource from '../../../generated/site/site-launch-state.json';
...
// Read from generated/site/site-registry.json, never from packages/catalog/*.
// A bundler inlines whatever this file imports, so importing the internal
// registry would ship per-family evidence blocks, release commits, internal
// proof-file paths and supplier posture to every browser. The projection is
// generated by scripts/generate-discovery.mjs; adding a field there is a
// deliberate decision to publish it.
const liveRegistry = siteRegistrySource as unknown as {
  observedAt: string;
  releaseId: string;
  products: {
    ...
    expectedReturnAt: string | null;
    publiclyReachable: boolean;
    observedQuote: ObservedProduct['observedPrice'];
    freeEntry: ObservedProduct['freeEntry'];
  }[];
};

export const observedTruth: ObservedTruth = {
  provenance: {
    observedAt: liveRegistry.observedAt,
    releaseId: liveRegistry.releaseId,
  } as ObservedTruth['provenance'],
  products: liveRegistry.products.map((product) => ({
    id: product.id,
    label: product.label,
    operations: product.operations,
    lifecycleState: product.state,
    // Derived from observed reachability, not from an internal maturity grade:
    // a family we serve has a verified paid outcome behind it, and one we do not
    // serve has nothing to show. The reason a family is unavailable is a supply
    // matter between us and a supplier, so it is not projected.
    proofLevel: product.publiclyReachable ? 'paid_outcome_verified' : 'none',
    reason: null,
    expectedReturnAt: product.expectedReturnAt,
    publiclyReachable: product.publiclyReachable,
    observedPrice: product.observedQuote,
...
export const proofLabels: Record<ProofLevel, string> = {
  none: 'no settled payment yet',
  quote_observed_unpaid: 'price quoted on request',
