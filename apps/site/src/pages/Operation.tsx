...
export function Operation({ operationId, onPhase }: { operationId: string; onPhase(phase: ExperiencePhase): void }) {
  ...
  const runFixture = async () => {
    ...
    setNotice('Completed · design fixture only · no live receipt or transaction was created.');
  };
  ...
  return (
    <div className="b12-operation" data-operation-id={contract.id} data-execution-state={executionState} data-scenario={scenario}>
      ...
      <section className="s5-contract-strip" aria-label="Operation contract facts"><div className="s5-narrow s5-contract-strip-inner">
        ...
        <div className="s5-contract-stat"><small>Payment</small><strong>{contract.paymentBehavior}</strong></div>
        ...
        <div className="s5-contract-main">
          <section className="s5-contract-section" id="s5-overview">
            ...
            <div className="s5-two-col">
              ...
              <article className="s5-panel s5-boundary"><p className="s5-kicker">Boundary</p><h3>Unknown fields stay unknown.</h3><p>Action class, operation-specific health, latency, provider identity, and history are not inferred from family or fixture copy when current public truth does not bind them.</p><div className="s5-task-examples"><div className="s5-task-example">No hidden live execution from this page.</div><div className="s5-task-example">No fake quote, wallet, receipt, transaction, or settlement.</div><div className="s5-task-example">Availability and payment behavior come from current public artifacts.</div></div></article>
            </div>
          </section>
          ...
          <section className="s5-contract-section" id="s5-result">
            <SectionHead number="03" label="Returned result" title="Result, evidence, and receipt stay separate." copy="The page keeps the response schema, supporting evidence, and any payment receipt distinct." />
            ...
            <div className="s5-result-window"><div className="s5-code-head"><span>{resultTab} contract · canonical where bound</span></div>{resultTab === 'result' ? <JsonBlock value={contract.openapi.responseSchema} empty="No operation-specific public response schema is bound." /> : null}{resultTab === 'evidence' ? <div className="s5-prose-block"><strong>Evidence contract</strong><p>{contract.openapi.responseSchema === null ? 'No operation-level evidence schema is currently published.' : 'The mapped response schema declares the public result shape.'}</p><code>Family availability: {contract.observedFamilyLifecycle}</code></div> : null}{resultTab === 'receipt' ? <div className="s5-prose-block"><strong>Receipt contract</strong><p>{contract.openapi.responseSchema === null ? 'No operation-specific receipt schema is currently bound.' : 'The mapped OpenAPI response exposes a receipt object.'}</p><code>Replay the same key and body; reconcile unknown settlement before retry.</code></div> : null}</div>
          </section>
          ...
          <section className="s5-contract-section" id="s5-availability">
            ...
            <div className="s5-availability-grid"><div className="s5-availability-cell"><small>Operation lifecycle</small><strong>{contract.lifecycle}</strong><p>{contract.publicAvailable ? 'Current catalog marks this operation publicly available.' : 'No public execution path is published for this operation.'}</p></div><div className="s5-availability-cell"><small>Family availability</small><strong>{contract.observedFamilyLifecycle}</strong><p>Observed from the deployed family route.</p></div><div className="s5-availability-cell"><small>Operation health</small><strong>Not bound</strong><p>{contract.health}.</p></div><div className="s5-availability-cell"><small>Payment</small><strong>{contract.paymentBehavior}</strong><p>The returned 402 is binding for a paid request.</p></div></div>
          </section>
          ...
          <section className="s5-contract-section" id="s5-execution">
            <SectionHead number="06" label="Execution states" title="Completed, refused, and unresolved are different outcomes." copy="The fixture shows the normal request path, refusal before execution, and quarantine while settlement remains unknown." />
            <div className="s5-execution-shell"><div className="s5-execution-control"><p className="s5-kicker">Scenario · design fixture</p><h3>Run the operating contract visually.</h3><p>No API, provider, wallet, payment, or settlement action is performed.</p><div className="s5-scenario-tabs" role="tablist" aria-label="Operation outcome fixture">{(['verified', 'refused', 'unresolved'] as OperationScenario[]).map((value) => <button key={value} className={`s5-tab${scenario === value ? ' active' : ''}`} type="button" onClick={() => { setScenario(value); resetFixture(); }}>{value === 'verified' ? 'Completed' : `${value[0]?.toUpperCase()}${value.slice(1)}`}</button>)}</div><div className="s5-execution-actions"><button className="b12-button b12-button-primary b12-liquid" type="button" onClick={() => void runFixture()}>Execute fixture</button><button className="b12-button b12-button-secondary b12-liquid" type="button" onClick={resetFixture}>Reset</button></div><div className={`s5-execution-result ${executionState}`}>{notice}</div></div><div className="s5-pipeline" aria-label="Request lifecycle">{pipeline.map(([step, label, copy]) => { const state = pipelineState(step); return <div key={step} className={`s5-pipeline-step ${step} ${state}`}><i /><div><strong>{label}</strong><p>{copy}</p></div><span>{state || 'waiting'}</span></div>; })}</div></div>
          </section>
          ...
          <section className="s5-contract-section" id="s5-proof">
            <SectionHead number="08" label="Receipt and recovery" title="The client keeps retry safety explicit." copy="A completed result can replay under the same key. Unknown settlement must be reconciled before any new authorization." />
            <div className="s5-proof-grid"><article className="s5-proof-panel"><p className="s5-kicker">Operation contract</p><h3>{contract.id}</h3><code className="s5-proof-id">{contract.publicRoute ?? 'No public route'}</code><div className="s5-proof-fields"><div className="s5-proof-field"><span>availability</span><strong>{contract.observedFamilyLifecycle}</strong></div><div className="s5-proof-field"><span>response schema</span><strong>{contract.openapi.responseSchema === null ? 'not bound' : 'OpenAPI-bound'}</strong></div><div className="s5-proof-field"><span>payment</span><strong>{contract.paymentBehavior}</strong></div></div></article><article className="s5-proof-panel"><p className="s5-kicker">Recovery</p><h3>Reuse or reconcile.</h3><code className="s5-proof-id">same key · same body</code><div className="s5-proof-fields"><div className="s5-proof-field"><span>completed</span><strong>return durable result</strong></div><div className="s5-proof-field"><span>conflict</span><strong>409; no execution</strong></div><div className="s5-proof-field"><span>unknown settlement</span><strong>reconcile before retry</strong></div></div></article></div>
          </section>
