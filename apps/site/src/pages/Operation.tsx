      ...
      <div className="s5-narrow s5-contract-layout">
        ...
        <div className="s5-contract-main">
          ...
          <section className="s5-contract-section" id="s5-proof">
            ...
            <div className="s5-proof-grid"><article className="s5-proof-panel"><p className="s5-kicker">Current proof boundary</p><h3>{contract.proofLabel}</h3><code className="s5-proof-id">family observation · {contract.familyLabel}</code><div className="s5-proof-fields"><div className="s5-proof-field"><span>operation</span><strong>{contract.id}</strong></div><div className="s5-proof-field"><span>response schema</span><strong>{contract.openapi.responseSchema === null ? 'not bound' : 'OpenAPI-bound'}</strong></div><div className="s5-proof-field"><span>receipt generation here</span><strong>none</strong></div></div></article><article className={`s5-proof-panel${proof === null ? '' : ' earned'}`}><p className="s5-kicker">Operation-specific proof</p><h3>{proof === null ? 'No settled payment bound to this operation.' : 'Settled payment on record.'}</h3><code className="s5-proof-id">{proof === null ? 'No gold production claim' : `${proof.amountDisplay} · ${proof.network} · ${proof.asset}`}</code><div className="s5-proof-fields">{proof === null ? <><div className="s5-proof-field"><span>settlement</span><strong>not asserted</strong></div><div className="s5-proof-field"><span>replay</span><strong>not asserted</strong></div><div className="s5-proof-field"><span>receipt</span><strong>not invented</strong></div></> : <><div className="s5-proof-field"><span>settlement confirmed</span><strong>{String(proof.settlementConfirmed)}</strong></div><div className="s5-proof-field"><span>useful result</span><strong>{String(proof.usefulResult)}</strong></div><div className="s5-proof-field"><span>same receipt replay</span><strong>{String(proof.replaySameReceipt)}</strong></div><div className="s5-proof-field"><span>second charge</span><strong>{String(proof.secondCharge)}</strong></div><div className="s5-proof-field"><span>replay charge</span><strong>never twice</strong></div></>}</div></article></div>
          </section>
