...
import { createN427sSourceAdapters, sourceQualifications } from '../../../infra/n4.27s/source-adapters.mjs';

const PORT = Number(process.env.PORT ?? '8080');
const DATA_ROOT = process.env.CLERVO_N427S_DATA_ROOT ?? '/var/lib/clervo-n427s/cache';
const MEILI_ENDPOINT = process.env.CLERVO_N427S_MEILI_ENDPOINT ?? 'http://clervo-n427s-meilisearch:7700/';
const MEILI_MASTER_KEY_FILE = process.env.CLERVO_N427S_MEILI_MASTER_KEY_FILE ?? '/var/run/clervo-n427s-meili/masterKey';
const MAXIMUM_OPERATIONS = Number(process.env.CLERVO_N427S_MAXIMUM_OPERATIONS ?? '900');
const MAXIMUM_CONCURRENT_ROUTES = Number(process.env.CLERVO_N427S_MAXIMUM_CONCURRENT_ROUTES ?? '2');
const OPERATION_COST_CEILING_USD = Number(process.env.CLERVO_N427S_OPERATION_COST_CEILING_USD ?? '0.002');
const USER_AGENT = 'Clervo-N4.27S-Staging/1.0 (mo@clervo.dev)';
...
const seedProbes = Object.freeze([
  ...
  ['developer_registry', 'npm package zod schema validation'],
  ['wikimedia', 'Open data'],
]);
...
function record(event) {
  ...
  process.stdout.write(`${JSON.stringify({ schemaVersion: 'clervo.n4.27s.monitoring-event.v1', ...safe })}\n`);
}
...
async function createRuntime() {
  ...
  const cache = new DurableRetrievalCache(cacheStore, 'n427s_staging', 1_500_000, 3_600_000);
  ...
  const sourceAdapters = createN427sSourceAdapters({ transport: cachedTransport, userAgent: USER_AGENT, mailto: 'mo@clervo.dev' });
  if ((await index.listDocuments().catch(() => [])).length === 0) {
    await meiliTransport({ method: 'POST', path: '/indexes', headers: { authorization: `Bearer ${masterKey}`, 'content-type': 'application/json' }, body: { uid: 'clervo_n427s_focused', primaryKey: 'documentId' } }).catch(() => undefined);
    const documents = [];
...
const server = http.createServer(async (request, response) => {
  ...
  } catch (error) { json(response, 503, { code: error instanceof Error ? error.message : 'n427s_staging_failure', lifecycle: 'unavailable', retryable: true }); }
});
server.listen(PORT, '0.0.0.0', () => record({ event: 'n427s_staging_started', state: 'ready', code: sha256(`${state.startedAt}:${PORT}`).slice(0, 24) }));
